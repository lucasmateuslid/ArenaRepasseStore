...
const SearchableFipeSelect: React.FC<SearchableFipeSelectProps> = ({ 
  ...
  const handleSelect = (code: string, name: string) => {
    ...
    setSearch(''); // Reseta busca mas mantém label visivel no placeholder condicional ou estado
    setIsOpen(false);
    onChange(code);
  };
            ...
            <div className="bg-brand-surface border border-gray-800 rounded-2xl p-5">
              ...
              {currentStatus === 'sold' && (
                <div className="mt-4 space-y-3 animate-fade-in bg-green-500/5 p-3 rounded-xl border border-green-500/20">
                  <div><label className="text-[10px] font-bold text-green-500 uppercase mb-1 block">Valor Final de Venda (R$)</label><input type="number" step="0.01" required className="w-full bg-black/30 border border-green-500/30 rounded-lg p-2 text-sm text-white focus:border-green-500 outline-none" placeholder="0.00" value={carFormData.soldPrice || ''} onChange={e => setCarFormData({...carFormData, soldPrice: Number(e.target.value)})} /></div>
                  <div><label className="text-[10px] font-bold text-green-500 uppercase mb-1 block">Data da Venda</label><input type="date" required className="w-full bg-black/30 border border-green-500/30 rounded-lg p-2 text-sm text-white focus:border-green-500 outline-none" value={carFormData.soldDate || new Date().toISOString().split('T')[0]} onChange={e => setCarFormData({...carFormData, soldDate: e.target.value})} /></div>
                  <div><label className="text-[10px] font-bold text-green-500 uppercase mb-1 block">Consultor Responsável</label><select required className="w-full bg-black/30 border border-green-500/30 rounded-lg p-2 text-sm text-white focus:border-green-500 outline-none" value={carFormData.soldBy || ''} onChange={e => setCarFormData({...carFormData, soldBy: e.target.value})}><option value="">Selecione...</option>{sellers.map((s: Seller) => <option key={s.id} value={s.name}>{s.name}</option>)}</select></div>
                </div>
              )}
            </div>
            <div className="bg-brand-surface border border-gray-800 rounded-2xl p-5">
              ...
              <div className="grid grid-cols-4 gap-2 max-h-40 overflow-y-auto custom-scrollbar">
                {carFormData.gallery?.map((imgUrl, idx) => (
                  <div key={`old-${idx}`} className="relative aspect-square group"><img src={imgUrl} className="w-full h-full object-cover rounded-lg border border-gray-700" alt={`thumb-${idx}`}/><button type="button" onClick={() => handleRemoveGalleryImage(idx)} className="absolute -top-1 -right-1 bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] shadow-lg hover:scale-110 transition z-10"><FaTimes/></button></div>
                ))}
                {galleryFiles.map((file, idx) => (
                  <div key={`new-${idx}`} className="relative aspect-square group"><img src={URL.createObjectURL(file)} className="w-full h-full object-cover rounded-lg border border-green-500/50" alt={`new-${idx}`}/><div className="absolute inset-0 border-2 border-green-500 rounded-lg pointer-events-none opacity-50"></div><button type="button" onClick={() => handleRemoveNewGalleryFile(idx)} className="absolute -top-1 -right-1 bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] shadow-lg hover:scale-110 transition z-10"><FaTimes/></button></div>
                ))}
              </div>
            </div>
          </div>
            ...
            <div className="bg-brand-surface border border-gray-800 rounded-2xl p-6 md:p-8 space-y-6">
              <div className="bg-blue-900/10 border border-blue-500/20 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div><h4 className="text-sm font-bold text-blue-100">Ficha Técnica FIPE</h4><p className="text-[10px] text-blue-300">Selecione para preencher automaticamente</p></div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {/* Campos de pesquisa FIPE substituidos */}
                    <SearchableFipeSelect 
                      options={fipeBrands}
                      onChange={onFipeBrand}
                      disabled={false}
                      placeholder="1. Marca"
                      stepLabel="Marca"
                    />
                    ...
                    <SearchableFipeSelect 
                      key={selectedBrandCode || 'model-select'} // Reset quando a marca muda
                      options={fipeModels}
                      onChange={onFipeModel}
                      disabled={fipeModels.length === 0}
                      placeholder="2. Modelo"
                      stepLabel="Modelo"
                    />
                    
                    <SearchableFipeSelect 
                      key={selectedModelCode || 'year-select'} // Reset quando o modelo muda
                      options={fipeYears}
                      onChange={onFipeYear}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  {[ 
                    ...
                    <div key={field.key} className="space-y-1.5">
                      <label className="text-[10px] font-bold text-gray-500 uppercase">{field.label} {field.hint && <span className="text-gray-600 normal-case ml-1">{field.hint}</span>}</label>
                      <div className="relative">
                        {field.type === 'select' ? 
                          ...
                          <input type={field.type === 'number' ? 'number' : 'text'} step={field.type === 'number' ? "any" : undefined} placeholder={field.placeholder || ''} className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-2.5 text-sm text-white focus:border-brand-orange outline-none" value={(carFormData as any)[field.key] || ''} onChange={e => { const val = field.key === 'licensePlate' ? e.target.value.toUpperCase() : e.target.value; setCarFormData({...carFormData, [field.key]: val}) }} maxLength={field.key === 'licensePlate' ? 8 : undefined} />
                        }
              ...
              <div className="grid grid-cols-2 gap-5 pt-4 border-t border-gray-800">
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-brand-orange uppercase">Preço Venda (R$)</label><input type="number" step="0.01" className="w-full bg-black/30 border border-brand-orange/50 rounded-lg px-3 py-3 text-lg font-bold text-white focus:border-brand-orange outline-none" placeholder="0.00" value={carFormData.price || ''} onChange={e => setCarFormData({...carFormData, price: Number(e.target.value)})} /></div>
                  <div className="space-y-1.5"><label className="text-[10px] font-bold text-gray-500 uppercase">Tabela FIPE (R$)</label><input type="number" step="0.01" className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-3 text-lg font-bold text-gray-400 focus:border-blue-500 outline-none" placeholder="0.00" value={carFormData.fipeprice || ''} onChange={e => setCarFormData({...carFormData, fipeprice: Number(e.target.value)})} /></div>
              </div>
              <div className="space-y-1.5"><label className="text-[10px] font-bold text-gray-500 uppercase">Descrição</label><textarea className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-3 text-sm text-white h-24 focus:border-brand-orange outline-none resize-none" value={carFormData.description || ''} onChange={e => setCarFormData({...carFormData, description: e.target.value})}></textarea></div>
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'financial' && (
        <div className="space-y-6 animate-fade-in">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-brand-surface border border-gray-800 p-5 rounded-2xl flex flex-col justify-between"><span className="text-[10px] font-bold text-gray-500 uppercase">Valor de Compra (Entrada)</span><div className="flex items-center gap-2 mt-2"><FaMoneyBillWave className="text-gray-500"/><input type="number" step="0.01" className="bg-transparent text-xl font-black text-white w-full outline-none placeholder-gray-700" placeholder="0.00" value={carFormData.purchasePrice || ''} onChange={e => setCarFormData({...carFormData, purchasePrice: Number(e.target.value)})} /></div></div>
            <div className="bg-brand-surface border border-gray-800 p-5 rounded-2xl flex flex-col justify-between"><span className="text-[10px] font-bold text-gray-500 uppercase">Total Despesas</span><div className="flex items-center gap-2 mt-2"><FaTools className="text-orange-500"/><span className="text-xl font-black text-white">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(financialSummary.totalExpenses)}</span></div></div>
            <div className="bg-brand-surface border border-gray-800 p-5 rounded-2xl flex flex-col justify-between"><span className="text-[10px] font-bold text-gray-500 uppercase">Custo Total (Entrada + Gastos)</span><span className="text-xl font-black text-white mt-2">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(financialSummary.totalCost)}</span></div>
            <div className={`border p-5 rounded-2xl flex flex-col justify-between shadow-lg relative overflow-hidden ${financialSummary.profit >= 0 ? 'bg-green-900/20 border-green-500/50' : 'bg-red-900/20 border-red-500/50'}`}><span className={`text-[10px] font-bold uppercase z-10 ${financialSummary.profit >= 0 ? 'text-green-500' : 'text-red-500'}`}>{currentStatus === 'sold' ? 'Lucro Realizado' : 'Lucro Projetado'}</span><div className="flex flex-col z-10"><span className={`text-2xl font-black ${financialSummary.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(financialSummary.profit)}</span><span className={`text-xs font-bold ${financialSummary.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>ROI: {financialSummary.roi.toFixed(1)}%</span></div>{financialSummary.profit >= 0 ? <FaChartLine className="absolute -bottom-4 -right-4 text-8xl text-green-500/10"/> : <FaChartLine className="absolute -bottom-4 -right-4 text-8xl text-red-500/10 transform rotate-180"/>}</div>
          </div>
          <div className="bg-brand-surface border border-gray-800 rounded-2xl p-4 md:p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 bg-black/20 p-4 rounded-xl border border-gray-800 mb-6">
               <div className="lg:col-span-2 space-y-1"><label className="text-[10px] font-bold text-gray-500 uppercase">Descrição</label><input type="text" className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:border-brand-orange outline-none" placeholder="Ex: Funilaria porta direita" value={newExpense.description} onChange={e => setNewExpense({...newExpense, description: e.target.value})} /></div>
               <div className="space-y-1"><label className="text-[10px] font-bold text-gray-500 uppercase">Tipo</label><select className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white outline-none" value={newExpense.type} onChange={e => setNewExpense({...newExpense, type: e.target.value as any})}><option value="maintenance">Manutenção</option><option value="repair">Reparo</option><option value="document">Documentação</option><option value="other">Outros</option></select></div>
               <div className="space-y-1"><label className="text-[10px] font-bold text-gray-500 uppercase">Valor (R$)</label><input type="number" step="0.01" className="w-full bg-black/30 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:border-brand-orange outline-none" placeholder="0.00" value={newExpense.amount || ''} onChange={e => setNewExpense({...newExpense, amount: Number(e.target.value)})} /></div>
               <div className="flex items-end sm:col-span-2 lg:col-span-1"><button type="button" onClick={handleAddExpense} className="w-full h-10 bg-brand-orange hover:bg-red-600 text-white font-bold rounded-lg text-xs uppercase flex items-center justify-center gap-2 transition"><FaPlus/> Adicionar</button></div>
            </div>
            <div className="overflow-x-auto">
               <table className="w-full text-left border-collapse min-w-[600px] md:min-w-full">
                 <thead><tr className="text-[10px] uppercase text-gray-500 border-b border-gray-800"><th className="py-2 px-4">Data</th><th className="py-2 px-4">Descrição</th><th className="py-2 px-4">Tipo</th><th className="py-2 px-4">Valor</th><th className="py-2 px-4 text-right">Ação</th></tr></thead>
                 <tbody className="text-sm">
                    {!carFormData.expenses || carFormData.expenses.length === 0 ? (<tr><td colSpan={5} className="py-8 text-center text-gray-500 italic">Nenhum gasto registrado.</td></tr>) : (carFormData.expenses.map((exp) => (<tr key={exp.id} className="border-b border-gray-800/50 hover:bg-white/5 transition"><td className="py-3 px-4 text-gray-400 font-mono text-xs">{new Date(exp.date).toLocaleDateString('pt-BR')}</td><td className="py-3 px-4 text-white font-medium">{exp.description}</td><td className="py-3 px-4"><span className={`text-[10px] uppercase font-bold px-2 py-1 rounded ${exp.type === 'maintenance' ? 'bg-blue-500/10 text-blue-500' : exp.type === 'document' ? 'bg-purple-500/10 text-purple-500' : exp.type === 'repair' ? 'bg-orange-500/10 text-orange-500' : 'bg-gray-700 text-gray-400'}`}>{exp.type}</span></td><td className="py-3 px-4 text-white font-bold">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(exp.amount)}</td><td className="py-3 px-4 text-right"><button type="button" onClick={() => handleRemoveExpense(exp.id)} className="text-red-500 hover:text-white transition p-2 bg-red-500/10 rounded-lg hover:bg-red-500"><FaTrash size={12}/></button></td></tr>)))}
                 </tbody>
               </table>
            </div>
          </div>
