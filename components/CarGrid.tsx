...
export const CarGrid: React.FC<CarGridProps> = ({ 
  ...
  if (loading && cars.length === 0) {
    return (
      <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6" : "space-y-4"}>
        {[1,2,3,4,5,6,7,8].map(i => <div key={i} className={`bg-brand-surface rounded-3xl animate-pulse border border-gray-800 ${viewMode === 'grid' ? 'h-[400px]' : 'h-[180px]'}`}></div>)}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6" : "space-y-6"}>
        {visibleCars.map((car) => {
            ...
            >
              ...
              <div className="p-4 flex-1 flex flex-col justify-between">
                <div className="mb-4">
                  ...
                  <h3 className="text-base md:text-lg font-black text-white truncate group-hover:text-brand-orange transition-colors">{car.model}</h3>
                  <div className="flex flex-wrap gap-x-3 gap-y-2 mt-3">
                    <div className="flex items-center gap-1.5 text-[10px] text-gray-400 font-bold bg-black/20 px-2 py-1 rounded-lg border border-gray-800/50">
                      <FaCalendarAlt className="text-brand-orange text-[9px]"/> {car.year === 32000 ? 'Zero KM' : car.year}
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px] text-gray-400 font-bold bg-black/20 px-2 py-1 rounded-lg border border-gray-800/50">
                      <FaTachometerAlt className="text-brand-orange text-[9px]"/> {car.mileage.toLocaleString()} KM
                    </div>
                    <div className="flex items-center gap-1.5 text-[10px] text-gray-400 font-bold bg-black/20 px-2 py-1 rounded-lg border border-gray-800/50">
                      <FaMapMarkerAlt className="text-brand-orange text-[9px]"/> {car.location || 'Arena Repasse'}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto pt-3 border-t border-gray-800/30">
                  ...
                  <div className="flex gap-2">
                    <button onClick={(e) => { e.stopPropagation(); openModal(car); }} className="flex-1 h-9 bg-white/5 border border-gray-800 rounded-xl text-[9px] font-black uppercase text-white hover:bg-white/10 transition shadow-sm">Detalhes</button>
                    <button onClick={(e) => { e.stopPropagation(); handleWhatsApp(car); }} className="w-9 h-9 bg-green-600 rounded-xl flex items-center justify-center text-white hover:bg-green-500 transition shadow-lg active:scale-95 transform"><FaWhatsapp size={16}/></button>
                  </div>
                </div>
              </div>
            </div>
